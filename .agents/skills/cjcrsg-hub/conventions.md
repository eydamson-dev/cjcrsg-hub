...
1. **Single Responsibility** - One component/hook per file
2. **DRY (Don't Repeat Yourself)** - Extract shared logic
3. **Early Returns** - Handle edge cases first
4. **Descriptive Names** - `getAttendees` not `getData`
5. **Default Exports** - Use named exports (easier refactoring)
6. **Avoid `any`** - Use proper types
7. **Handle Errors** - Always have error boundaries
8. **Mobile First** - Test responsive design
9. **Performance** - Use `useMemo`, `useCallback` when needed
10. **Accessibility** - Use proper ARIA labels
